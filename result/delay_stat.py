...
def stat_result2(xml_file):
	...
	Sum_CO2 = 0
	Sum_fuel = 0
	timeloss_total = [ [] for i in range(int(3600/120) + 1)]
	Number_delay= 0
	Number_emission = 0
...
if __name__ == '__main__':
	averageB,total_averageB  = stat_result2("tripinfo10B.xml")

	average1,total_average1  = stat_result2("tripinfo10.xml")


	average2,total_average2= stat_result2("tripinfo102.xml")

	average3,total_average3= stat_result2("tripinfo103.xml")	
	#average33 =  stat_result2("tripinfo33.xml")
	#print(average1,averageB)

	ln0,= plt.plot(range(len(averageB)),averageB,c='r')

	ln1, = plt.plot(range(len(average1)),average1,c='g')

	ln2, = plt.plot(range(len(average2)),average2,c='blue')

	ln3, = plt.plot(range(len(average3)),average3,c='black')
	#ln4, = plt.plot(range(len(average33)),average33,c='yellow')
	plt.legend(handles=[ln0,ln1,ln2,ln3], labels=['Benchmark Delay:%s'%total_averageB,'Case120-1 Delay:%s'%total_average1,'Case120-2 Delay:%s'%total_average2 ,'Case120-3 Delay:%s'%total_average3 ],
	loc='lower left')
	#plt.legend(handles=[ln0,ln1], labels=['Benchmark Delay:%s'%total_averageB,'Case120-1 Delay:%s'%total_average1]) 
	plt.show() 
